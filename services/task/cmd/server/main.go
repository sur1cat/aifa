...
import (
	...
	"github.com/sur1cat/aifa/task-service/internal/config"
	"github.com/sur1cat/aifa/task-service/internal/events"
	"github.com/sur1cat/aifa/task-service/internal/handler"
	"github.com/sur1cat/aifa/task-service/internal/jwt"
	"github.com/sur1cat/aifa/task-service/internal/middleware"
	"github.com/sur1cat/aifa/task-service/internal/migrate"
...
func main() {
	...
	if err := migrate.Run(ctx, pool, "migrations", "tasks"); err != nil {
	...
	sub, err := events.NewSubscriber(nc, taskRepo)
	if err != nil {
	...
	taskHandler := handler.NewTaskHandler(taskRepo, cfg.DefaultCurrency)
	...
	protected.POST("/tasks/:id/toggle", taskHandler.Toggle)
	...
	go func() {
		log.Info("task-service listening", "port", cfg.Port)
...
func connectInfra(ctx context.Context, cfg *config.Config) (*pgxpool.Pool, *redis.Client, *nats.Conn, error) {
	...
	g.Go(func() error {
		c, err := nats.Connect(cfg.NATSURL, nats.Name("task-service"), nats.MaxReconnects(-1))
		if err != nil {
			return err
		}
		nc = c

...
func (h *TaskHandler) Create(c *gin.Context) {
	...
	if priority == "" {
		...
		respondError(c, http.StatusBadRequest, codeValidation, "invalid priority (low|medium|high|urgent)")
		return
	}
	dueDate := req.DueDate
	if dueDate == "" {
		dueDate = time.Now().Format("2006-01-02")
	} else if _, err := time.Parse("2006-01-02", dueDate); err != nil {
		respondError(c, http.StatusBadRequest, codeValidation, "due_date must be YYYY-MM-DD")
		return
	}
...
func applyUpdate(t *domain.Task, req updateRequest) error {
	...
	if req.Priority != "" {
		...
		if !p.Valid() {
			return errors.New("invalid priority (low|medium|high|urgent)")
		}
		t.Priority = p
	}
	if req.DueDate != "" {

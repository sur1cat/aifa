...
// Claims must stay in sync with auth-service's claim shape. The JWT is
// signed by auth-service with the shared JWT_SECRET; every other service
// validates independently — no round-trip to auth-service per request.
type Claims struct {

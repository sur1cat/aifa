...
class ReceiptOCRUnitTests(unittest.TestCase):
    ...
    def test_extract_date_text_month_russian(self) -> None:
        self.assertEqual(
            receipt_ocr._extract_date(["Дата: 15 марта 2025"]),
            "2025-03-15",
        )
        self.assertEqual(
            receipt_ocr._extract_date(["7 января 2024, 09:30"]),
            "2024-01-07T09:30:00",
        )
        self.assertEqual(
            receipt_ocr._extract_date(["Чек от 3 декабря 2023 г."]),
            "2023-12-03",
        )

    def test_extract_date_text_month_english(self) -> None:
        self.assertEqual(
            receipt_ocr._extract_date(["Date: March 15, 2025"]),
            "2025-03-15",
        )
        self.assertEqual(
            receipt_ocr._extract_date(["15 March 2025 14:22:00"]),
            "2025-03-15T14:22:00",
        )
        self.assertEqual(
            receipt_ocr._extract_date(["Jan 1, 2024"]),
            "2024-01-01",
        )
    ...
    @patch("app.receipt_ocr._load_image")
    def test_scan_receipt_returns_normalized_result(
        ...
        mock_load_image.return_value = SimpleNamespace()  # dummy Image object

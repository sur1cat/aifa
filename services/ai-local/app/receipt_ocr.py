...
def _extract_text(image_bytes: bytes, languages: str) -> str:
    try:
        import pytesseract
...
def _extract_header_candidates(image_bytes: bytes, languages: str) -> list[str]:
...
def _extract_footer_candidates(image_bytes: bytes, languages: str) -> list[str]:
    return _extract_zone_candidates(
        image_bytes,
        languages,
        top_ratio=0.68,
...
def _extract_total_candidates(image_bytes: bytes, languages: str) -> list[str]:
    return _extract_zone_candidates(
        image_bytes,
        languages,
        top_ratio=0.5,
...
def scan_receipt(image_bytes: bytes, languages: str | None = None) -> ReceiptScanResult:
    if languages is None:
        languages = os.getenv("OCR_LANGS", "eng+rus+kaz")
    raw_text = _extract_text(image_bytes, languages)
    lines = _normalize_lines(raw_text)
    header_candidates = _extract_header_candidates(image_bytes, languages)
    header_lines: list[str] = []
    for text in header_candidates:
        header_lines.extend(_normalize_lines(text))
    footer_candidates = _extract_footer_candidates(image_bytes, languages)
    footer_lines: list[str] = []
    for text in footer_candidates:
        footer_lines.extend(_normalize_lines(text))
    total_candidates = _extract_total_candidates(image_bytes, languages)

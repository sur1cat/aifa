...
func (h *TransactionHandler) Update(c *gin.Context) {
	...
	if req.Amount != 0 {
		if req.Amount < 0 {
			respondError(c, http.StatusBadRequest, codeValidation, "amount must be positive")
			return
		}
		t.Amount = req.Amount
	}
	if req.Type != "" {
		tt := domain.TransactionType(req.Type)
		if tt != domain.TypeIncome && tt != domain.TypeExpense {
			respondError(c, http.StatusBadRequest, codeValidation, "type must be income or expense")
			return
		}
		t.Type = tt
	}
	...
	if req.Date != "" {
		if _, err := time.Parse("2006-01-02", req.Date); err != nil {
			respondError(c, http.StatusBadRequest, codeValidation, "date must be YYYY-MM-DD")
			return
		}
		t.Date = req.Date
	}

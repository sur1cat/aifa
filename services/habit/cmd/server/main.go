...
import (
	...
	"github.com/sur1cat/aifa/habit-service/internal/config"
	"github.com/sur1cat/aifa/habit-service/internal/events"
	"github.com/sur1cat/aifa/habit-service/internal/handler"
	"github.com/sur1cat/aifa/habit-service/internal/jwt"
	"github.com/sur1cat/aifa/habit-service/internal/middleware"
	"github.com/sur1cat/aifa/habit-service/internal/migrate"
...
func main() {
	...
	if err := migrate.Run(ctx, pool, "migrations", "habits"); err != nil {
	...
	sub, err := events.NewSubscriber(nc, habitRepo)
	if err != nil {
	...
	habitHandler := handler.NewHabitHandler(habitRepo, cfg.DefaultCurrency)
	...
	protected.POST("/habits/:id/toggle", habitHandler.Toggle)
	...
	go func() {
		log.Info("habit-service listening", "port", cfg.Port)
...
func connectInfra(ctx context.Context, cfg *config.Config) (*pgxpool.Pool, *redis.Client, *nats.Conn, error) {
	...
	g.Go(func() error {
		c, err := nats.Connect(cfg.NATSURL, nats.Name("habit-service"), nats.MaxReconnects(-1))
		if err != nil {
			return err
		}
		nc = c

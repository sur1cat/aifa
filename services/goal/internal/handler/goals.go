...
func (h *GoalHandler) Create(c *gin.Context) {
	...
	if req.TargetAmount != nil && *req.TargetAmount <= 0 {
		respondError(c, http.StatusBadRequest, codeValidation, "target_amount must be positive")
		return
	}
	if current < 0 {
		respondError(c, http.StatusBadRequest, codeValidation, "current_amount must be non-negative")
		return
	}
...
func (h *GoalHandler) Update(c *gin.Context) {
	...
	if req.CurrentAmount != nil && g.CurrentAmount != prevAmount && h.publisher != nil {
		evt := events.GoalProgress{
			GoalID:        g.ID.String(),
			UserID:        userID.String(),
			CurrentAmount: g.CurrentAmount,
			Completed:     g.Progress() >= 1,
		}
		if g.TargetAmount != nil {
			evt.TargetAmount = *g.TargetAmount
		}
		h.publisher.PublishGoalProgress(evt)
	}
...
func applyUpdate(g *domain.Goal, req updateRequest) error {
	...
	if req.CurrentAmount != nil {
		if *req.CurrentAmount < 0 {
			return errors.New("current_amount must be non-negative")
		}

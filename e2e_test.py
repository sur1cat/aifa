...
def test_parse_message(token):
    ...
    # 9. Относительные даты в транзакциях
    yesterday  = (date.today() - timedelta(days=1)).isoformat()
    day_before = (date.today() - timedelta(days=2)).isoformat()
    today_iso  = date.today().isoformat()

    d9 = pm("купил кофе вчера за 800 тг")
    check("9: 'вчера' → tx_date=yesterday", d9.get("transaction", {}).get("date") == yesterday)

    d10 = pm("потратил 500 тг позавчера на такси")
    check("10: 'позавчера' → tx_date=day_before", d10.get("transaction", {}).get("date") == day_before)

    d11 = pm("получил зарплату 200000 тг")
    check("11: без даты → tx_date=today", d11.get("transaction", {}).get("date") == today_iso)
...
def test_notification(token):
    section("Notification: push token register / unregister")

    NOTIF = SERVICES.get("notification", "http://localhost:8009")

    # Health
    resp, status = req("GET", NOTIF + "/health")
    check("notification /health → 200", status == 200)

    # Register a fake device token
    fake_token = "e2e_fake_device_token_" + str(int(time.time()))
    resp2, status2 = req("POST", NOTIF + "/push/register",
                         {"token": fake_token, "platform": "ios"}, token=token)
    check("POST /push/register → 200", status2 == 200)
    check("register status=registered", resp2.get("data", {}).get("status") == "registered")

    # Re-register same token (idempotent upsert)
    _, status3 = req("POST", NOTIF + "/push/register",
                     {"token": fake_token, "platform": "ios"}, token=token)
    check("POST /push/register idempotent → 200", status3 == 200)

    # Unregister
    resp4, status4 = req("POST", NOTIF + "/push/unregister",
                         {"token": fake_token}, token=token)
    check("POST /push/unregister → 200", status4 == 200)
    check("unregister status=unregistered", resp4.get("data", {}).get("status") == "unregistered")

    # Unregister without auth → 401
    _, status5 = req("POST", NOTIF + "/push/unregister", {"token": fake_token})
    check("POST /push/unregister no auth → 401", status5 == 401)


def test_user_profile(token):
    section("User: profile")

    # GET /users/me — профиль создаётся автоматически при первой авторизации
    resp, status = req("GET", SERVICES["user"] + "/users/me", token=token)
    check("GET /users/me → 200", status == 200)
    profile = resp.get("data", {})
    check("profile has id", bool(profile.get("id")))
    check("profile has phone", bool(profile.get("phone")))

    # PUT /users/me — обновить имя и валюту
    resp2, status2 = req("PUT", SERVICES["user"] + "/users/me", {
        "name": "E2E Test User",
        "currency": "KZT",
    }, token=token)
    check("PUT /users/me → 200", status2 == 200)
    check("name updated", resp2.get("data", {}).get("name") == "E2E Test User")

    # Повторный GET — убедиться что изменения сохранились
    resp3, status3 = req("GET", SERVICES["user"] + "/users/me", token=token)
    check("GET /users/me after update → 200", status3 == 200)
    check("name persisted", resp3.get("data", {}).get("name") == "E2E Test User")

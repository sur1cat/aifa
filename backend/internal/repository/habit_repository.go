...
func (r *HabitRepository) GetByUserID(ctx context.Context, userID uuid.UUID) ([]*domain.Habit, error) {
	...
	// Get all completions and progress values
	if len(habitIDs) > 0 {
...
// AddCompletion adds a completion for a habit
...
// RemoveCompletion removes a completion for a habit
...
// getCompletions returns all completion dates for a habit
...
// getCompletionsBatch returns all completion dates for multiple habits
...
// VerifyOwnership checks if a habit belongs to a user
...
// getProgressValues returns all progress values for a habit
...
// getProgressValuesBatch returns all progress values for multiple habits
...
// SetProgressValue sets or updates progress for a habit on a date
...
// RemoveProgressValue removes progress for a habit on a date

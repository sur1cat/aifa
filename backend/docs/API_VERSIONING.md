...
Atoma API uses **URL path versioning** as the primary versioning strategy.
...
```
https://api.azamatbigali.online/api/v{major}/*
```
...
2. **Add headers** to deprecated responses:
   ...
   Link: <https://api.azamatbigali.online/api/v2/habits>; rel="successor-version"
   ```
...
```swift
struct APIConfig {
    static let baseURL = "https://api.azamatbigali.online/api/v1"
}
```
...
enum APIVersion: String {
    ...
    var baseURL: String {
        "https://api.azamatbigali.online/api/\(rawValue)"
    }
}
```
